    ...
    {[1, 2].map((day) => (
      <div key={day} className="relative">
        ...
        <div className="grid gap-4 ml-4 pl-4 border-l-2 border-gray-100">
          {[1, 2, 3].map((ritual) => (
            <div key={ritual} className="bg-white p-5 rounded-xl border border-gray-100 flex gap-4">
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="space-y-8">
        {state.schedule.map((day, dayIdx) => (
          <div key={day.id} className="relative animate-slide-up" style={{ animationDelay: `${dayIdx * 0.1}s` }}>
            <div className="sticky top-16 z-10 bg-red-50/95 backdrop-blur-md p-4 rounded-xl border border-red-100 flex justify-between items-center mb-4 shadow-sm">
              <div>
                <h3 className="text-xl font-bold text-red-800 font-serif">{day.day}</h3>
                <p className="text-sm text-red-600">{new Date(day.date).toLocaleDateString('en-IN', { dateStyle: 'long' })}</p>
              </div>
            ...
            <div className="grid gap-4 ml-4 pl-4 border-l-2 border-red-200">
              {day.rituals.map((ritual, idx) => (
                ...
                >
                  ...
                  <div>
                    <h4 className="font-bold text-gray-900 text-lg group-hover:text-red-800 transition-colors font-serif">{ritual.title}</h4>
                    <p className="text-gray-600 text-sm mt-1">{ritual.description}</p>
                  </div>
